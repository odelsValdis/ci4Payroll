<script>
     var bosAfirmasiStore = new DevExpress.data.CustomStore({
        key: "id",
        load: function() {
            return $.getJSON(SERVICE_URL+"/getTrreportnew/3/"+<?php echo $branchId ?>+"/"+<?php echo $idValue ?>+"/"+<?php echo $idYear ?>); 
        ...
        update: function(key, values) {
            return $.ajax({
             url: SERVICE_URL + "/updateTrreport/3/"+<?php echo $branchId ?>+"/"+<?php echo $idValue ?>+"/"+<?php echo $idYear ?>,
             method: "POST",
             data: values,
         });
...
$(function(){
    $("#gridAfirmasi").dxDataGrid({
        dataSource:bosAfirmasiStore,
        editing: {
            mode: "batch",
            allowUpdating: true,
...
function getGridAfirmasi(){
    return $("#gridAfirmasi").dxDataGrid("instance");
}
</script>