<script>
  var bosRegularStore = new DevExpress.data.CustomStore({
        key: "id",
        load: function() {
            return $.getJSON(SERVICE_URL+"/getTrreportnew/1/"+<?php echo $branchId ?>+"/"+<?php echo $idValue ?>+"/"+<?php echo $idYear ?>); 
        ...
        update: function(key, values) {
            return $.ajax({
                url: SERVICE_URL + "/updateTrreport/1/"+<?php echo $branchId ?>+"/"+<?php echo $idValue ?>+"/"+<?php echo $idYear ?>,
             method: "POST",
             data: values,
         });
